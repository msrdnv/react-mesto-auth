...
export default function Header({loggedIn, email, onSignOut, isMenuActive, onClick}) {
  ...
  return (
    <header className={isMenuActive ? "header header_mobile" : "header"}>
      <img className="header__logo" src={logoMestoWhite} alt="Логотип Mesto Russia"/>
      <div className={isMenuActive ? "header__nav-container header__nav-container_mobile" : "header__nav-container"}>
        <p className={loggedIn ? (isMenuActive ? "header__email-info header__email-info_mobile" : "header__email-info") : "header__email-info header__email-info_disabled"}>
          {email}
        </p>
        <Link to="/signin" onClick={onSignOut} className={loggedIn ? (isMenuActive ? "header__signout-link header__signout-link_mobile" : "header__signout-link") : "header__signout-link header__signout-link_disabled"}>
          Выйти
        </Link>
